...
double StrausCalc::term(TokenStream & ts)
{
	...
	double d = 0;
	// считыааем и вычисляем первичное выражение
	...
	while (true)
	{
		...
		{
		case '*':
			// вычисляем первичное выражение и умножаем
			left *= extra(ts);
			t = ts.getToken();
			break;

		case '/':
			// вычисляем первичное выражение и делим
			d = extra(ts);
			if (d == 0.0)
			{
		...
		case '%':
			// вычисляем первичное выражение и остатка от целочисленного деления
			d = extra(ts);
			if (d == 0.0)
			{
				auto msg{ MESSAGE_ERROR_01.arg("% Divide by zero") };
...
double StrausCalc::extra(TokenStream & ts)
{
	...
	while (true) {
		...
		default:
			ts.putback(t);  // Помещает объект t обратно в поток лескем
			return left;
		}
	}
}
